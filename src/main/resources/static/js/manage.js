...
(function () {
    ...
    function renderPathRow(path) {
        const tr = document.createElement('tr');
        const idTd = document.createElement('td');
        idTd.textContent = path.id || '';
        const nameTd = document.createElement('td');
        nameTd.textContent = path.name || '';
    ...
    async function onChangePathStatus(path, willDisable, selectElem) {
        ...
        try {
            ...
            if (!resp.ok) {
                alert('状態の更新に失敗しました');
                try {
                    selectElem.value = prev ? 'disabled' : 'enabled';
                } catch (_) {
                }
                return;
            }
            ...
            if (!menus || !menus.length) {
                ...
                try {
                    ...
                    menuNames.forEach((name) => {
                        ...
                        idTd.textContent = '';
                        const nameTd = document.createElement('td');
                        nameTd.textContent = name + ' (画面管理由来)';
                        const actionsTd = document.createElement('td');
                        ...
                        importBtn.addEventListener('click', async () => {
                            // create menu via API
                            try {
                                const resp2 = await fetch(apiMenus, {
                                    method: 'POST',
                                    headers: getHeaders(),
    ...
    async function onAddScreen() {
        ...
        try {
            const resp = await fetch(apiContent, {
                method: 'POST',
                headers: getHeaders(),
    ...
    async function onAddToBlacklist() {
        ...
        if (!sel) {
            alert('セレクトボックスが見つかりません');
            return;
        }
        const ip = (sel.value || '').trim();
        if (!ip) {
            alert('IPアドレスを選択してください');
            return;
        }
        try {
    ...
    try {
        if (typeof window !== 'undefined') {
            window.__manage_onDeletePath = onDeletePath;
            window.__manage_onRestorePath = onRestorePath;
        }
    } catch (_) {
    }
